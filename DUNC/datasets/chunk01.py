...
class CUHK01(BaseDataset):
    ...
    def __init__(self, root='', verbose=True):
        ...
        if 'CUHK01' in self.dataset_dir:
            self.dataset_dir = op.join(root,'CUHK-PEDES')
    ...
    def _split_anno(self, anno_path: str):
        ...
        for anno in annos:
            if 'CUHK01' in anno['file_path']:
                train_annos.append(anno)

        train_annos.sort(key=lambda t: t['id'])
        for i in range(len(train_annos)):
            train_annos[i]['id']= i//4 + 1

        test_annos = train_annos[-100*4:]
        val_annos = train_annos[-100*8:-100*4]
        train_annos = train_annos[0:-100*8]
